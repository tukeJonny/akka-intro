...
class MsgRoutingTest extends TestKit(ActorSystem("MsgRoutingTest"))
  ...
  "The Router" must {
    "routes depending on speed" in {
      ...
      // カスタムプールであるSpeedRouterPoolを用いて、normalFlow, cleanupへと
      // リダイレクトするルーターを作成
      val router = system.actorOf(Props.empty.withRouter(
        new SpeedRouterPool(50,
          Props(new RedirectRouter(normalFlowProbe.ref)),
          Props(new RedirectRouter(cleanupProbe.ref)))
        ))
    ...
    "routes direct test" in {
      ...
      val route = ExtractRoute(router) // ルータを制御してテスト
