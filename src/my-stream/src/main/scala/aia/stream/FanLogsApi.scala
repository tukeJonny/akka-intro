...
class FanLogsApi(val logsDir: Path, val maxLine: Int, val maxJsObject: Int)(
  ...
  import akka.sream.{ FlowShape, Graph }
  import akka.sream.scaladsl.{ Broadcast, GraphDSL, RunnableGraph }
  ...
  def processStates(logId: String): FlowLike = {
    ...
    Flow.fromGraph(
      // GraphからFlowを作成し、POSTルート定義で用いる
      GraphDSL.create() { implicit builder => // build: GraphDSL.Builder
        import GraphDSL.Implicits._ // このスコープにGraphDSLのメソッドをインポート
        val bcast = builder.add(Broadcast[Event](5)) // グラフにBroadCastノードを追加
        val js = builder.add(jsFlow) // グラフに全イベントをJSON形式のまま渡すFlowを追加
        ...
        bcast ~> js.in // すべてのイベント
        bcast ~> ok ~> jsFlow ~> logFileSink(logId, Ok) // OKの場合のみ
        bcast ~> warning ~> jsFlow ~> logFileSink(logId, Warning) // Warning(ry)
        bcast ~> error ~> jsFlow ~> logFileSink(logId, Error) // Error(ry)
        bcast ~> critical ~> jsFlow ~> logFileSink(logId, Critical) // Critical(ry)

        // Broadcastの入り口とjsFlowの出口からFlowShapeグラフを作成
  ...
  // 状態がOKでない(=Warning, Error, Critical)ログをマージする
  def mergeNotOk(logId: String): Source[ByteString, NotUsed] = {
    ...
    Source.fromGraph(
      GraphDSL.create() { implicit builder =>
        ...
        // mergeを作り、そこに前ログレベルのログを流し込む
        val merge = builder.add(Merge[ByteString](3))
        warningShape ~> merge
        errorShape ~> merge
        criticalShape ~> merge

        // ログレベル毎 -> マージの一連のグラフをSourceShapeとして返す
      ...
      pathEndOrSingleSlash {
        post {
          entity(asSourceOf[Event]) { src =>
            onComplete(src.via(processStates(logId)).toMat(logFileSink(logId))(Keep.right).run()) {
  ...
  // Vectorにあるすべてのソースをマージ
  // 返るのはNoneも有りうるのでOption
