...
class ContentNegLogsApi(val logsDir: Path, val maxLine: Int, val maxJsObject: Int)(
      ...
      pathEndOrSingleSlash {
        post {
          entity(as[Source[Event, _]]) { src =>
            onComplete(src.via(outFlow).toMat(logFileSink(logId))(Keep.right).run()) {
        ...
        get {
          extractRequest { req =>
            if (Files.exists(logFile(logId))) {
              // toResponseForでAcceptヘッダに応じた処理を行える
