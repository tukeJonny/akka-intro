...
object ShopperSingleton {
  def props =
    Props(new ShoppersSingleton)
  def name =
    "shoppers-singleton"
}

class ShoppersSingleton extends Actor {
  val managerSettings = ClusterSingletonManagerSettings(context.system)
                          .withRole(None)
                          .withSingletonName(Shoppers.name)
  val singletonManager = context.system.actorOf(ClusterSingletonManager.props(Shoppers.props, PoisonPill, managerSettings))

  val proxySettings = ClusterSingletonProxySettings(context.system)
                          .withRole(None)
                          .withSingletonName("shoppers-proxy")
  val shoppers = context.system.actorOf(ClusterSingletonProxy.props(
